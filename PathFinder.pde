class PathFinder {
  ...
  PathFinder(int piece, int nextPiece, int[][][] board, int[] backgroundColor, float ah, float l, float h, float b, float nah, float nl, float nh, float nb, float f, float s) {
    ...
    this.factors1 = new float[4];
    this.factors2 = new float[4];
    ...
    this.heightFactor = 0.51;
    this.linesFactor = 0.76;
    this.holesFactor = 0.356;
    this.bumpFactor = 0.184;
  ...
  private void nextPiece(Position pos) {
    ...
    //this.heightFactor = this.factors2[0];
    //this.linesFactor = this.factors2[1];
    //this.holesFactor = this.factors2[2];
    //this.bumpFactor = this.factors2[3];
  }
  ...
  private void restoreCurrPiece() {
    ...
    //this.heightFactor = this.factors1[0];
    //this.linesFactor = this.factors1[1];
    //this.holesFactor = this.factors1[2];
    //this.bumpFactor = this.factors1[3];
  }
  ...
  private void findPossibleEndPositions() {
    ...
    /*
    // if has space above it, check if fallingPiece can be placed above it
    if ((r == rows || this.board[r][c]) && !this.board[r-1][c]) {
        for (int t = 0; t < numRotations; t++) {
          if (t > 0) rotateTheoreticalFallingPiece();
          fallingPieceHeight = fallingPiece.length; //assert > 0
          for (int h = 1; h <= fallingPieceHeight; h++) {
            if (isValidEndPos(r - h, c)) {
              endPositions.push(new Position(r - h, c, t));
            }
          }
        }
        this.fallingPiece = tetrisPieces[this.pieceID]; //reset rotation
